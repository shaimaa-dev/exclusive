    ...
    return (
        <div className='group relative z-30'>
            <Link to={`/products/${product.id}`}>
                ...
                {product.discountPercentage && <div className='bg-buttoncolor px-1 rounded-md absolute left-2 top-2 text-xl text-white '>{product.discountPercentage < 1 ? product.discountPercentage.toFixed(1) : product.discountPercentage.toFixed(0)}%</div>}
                <div className=' px-2 rounded-md gap-2 absolute right-2 top-2 text-xl text-black  '>
                    {showRemove &&
                        <button onClick={removeProductFromWishList} className="w-[30px] h-[30px] rounded-full bg-white flex items-center justify-center"><RiDeleteBin5Line /></button>
                    }
                    {showEye &&
            ...
            <div className='absolute top-[215px] w-[100%]'>
                <button className='btn block w-full mx-auto bg-black text-white py-2 transition-all duration-300 ease-in-out opacity-100 translate-y-0 capitalize lgl:opacity-0 lgl:translate-y-5 group-hover:opacity-100 group-hover:translate-y-0'>add to cart</button>
            </div>
        </div>
    )
}
